...
const css = readFileSync(path.join(__dirname, "../lineframe.css"), "utf8");
const accents = readFileSync(path.join(__dirname, "../lineframe-accents.css"), "utf8");

function token(name, mode = 0) {
  const values = [...css.matchAll(new RegExp(`${name}:\\s*(#[a-f\\d]{6})`, "gi"))];
  assert.ok(values[mode], `Missing ${name} for mode ${mode}`);
...
const palettes = [...accents.matchAll(
...
test("each named palette has a light and dark value", () => {
  const names = [...accents.matchAll(/\[data-lineframe-accent="([^"]+)"\]/g)]
    .map((match) => match[1]);
  assert.ok(names.length > 0);
  assert.deepEqual(palettes.map((match) => match[1]), names);
});
